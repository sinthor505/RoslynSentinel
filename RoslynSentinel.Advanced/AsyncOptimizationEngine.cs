...
public class AsyncOptimizationEngine
{
    ...
    public async Task<DocumentEditResult> OptimizeIndependentAwaitsAsync(FilePathWrapper filePath, string methodName, CancellationToken cancellationToken = default)
    {
        ...
        var newRoot = root!.ReplaceNode(methodNode, newMethodNode);
        return new DocumentEditResult { Outcome = EditOutcome.Modified, UpdatedText = RoslynFormattingHelper.NormalizeWholeSubtreeWhitespace(newRoot).ToFullString(), FilePath = filePath };
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new DocumentEditResult
        {
            ...
            UpdatedText = RoslynFormattingHelper.NormalizeWholeSubtreeWhitespace(newRoot2).ToFullString(),
            FilePath = filePath,
        };
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var rewrittenMethod = annotatedMethodNode.ReplaceNodes(
            ...
            (original, _) =>
            {
                ...
                if (newExpr == null)
                {
                    return (ExpressionSyntax)original;
                }
        ...
        var updatedText = RoslynFormattingHelper.NormalizeWholeSubtreeWhitespace(newRoot).ToFullString();
                ...
                .Select(al =>
                {
                    var kept = al.Attributes
                        .Where(a => a.Name.ToString() != MigrationCandidateShortName &&
                                    a.Name.ToString() != MigrationCandidateFullName)
                        .ToList();
                    if (kept.Count == al.Attributes.Count)
                    {
                        return al;
                    }
...
";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var methodNode = root.DescendantNodes()
                             .OfType<MethodDeclarationSyntax>()
                             .FirstOrDefault(m => m.Identifier.Text == methodName) ?? throw new InvalidOperationException(
                $"Method '{methodName}' not found in '{filePath}'. Names are case-sensitive.");
        ...
        var newSource = RoslynFormattingHelper.NormalizeWholeSubtreeWhitespace(newRoot).ToFullString();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var (filePath, indices) in byFile)
        {
            ...
            var finalSource = RoslynFormattingHelper.NormalizeWholeSubtreeWhitespace(root).ToFullString();
            ...
            if (!dryRun && flagged.Any(f => f.FilePath == document.FilePath!))
            {
                ...
                await Parallel.ForEachAsync(flaggedInThisFile, async (item, cancellationToken) =>
                {
                    ...
                    .FirstOrDefault(m => m.Identifier.Text == item.MethodName &&
                                         m.GetLocation().GetLineSpan().StartLinePosition.Line + 1 == item.Line) ?? rewrittenRoot.DescendantNodes()
                        .OfType<MethodDeclarationSyntax>()
                        .FirstOrDefault(m => m.Identifier.Text == item.MethodName);
                    if (methodNode == null)
                    {
                ...
                if (fileModified)
                {
                    fileChanges[document.FilePath!] = RoslynFormattingHelper.NormalizeWholeSubtreeWhitespace(rewrittenRoot).ToFullString();
                }
            }
        }).ConfigureAwait(false);
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        foreach (var inv in invocations)
        {
            ...
            if (semanticModel != null)
            {
                ...
                if (si.Symbol is IMethodSymbol ms0)
                {
                    candidates.Add(ms0);
                }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var doc in documents)
        {
            ...
            foreach (var attrList in root.DescendantNodes().OfType<AttributeListSyntax>())
            {
                foreach (var attr in attrList.Attributes)
                {
                    if (!IsMatchingMigrationAttr(attr, pattern))
                    {
                        continue;
                    }
                    ...
                    if (methodName != null && ownerMethodName != methodName)
                    {
                        continue;
                    }
    ...
    private sealed class MigrationCandidateRemover(string? pattern, string? methodName = null) : CSharpSyntaxRewriter
    {
        public override SyntaxNode? VisitAttributeList(AttributeListSyntax node)
        {
            ...
            var keep = node.Attributes
                ...
                .ToList();
            if (keep.Count == node.Attributes.Count)
            {
                return node;
            }
