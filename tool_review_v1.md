# RoslynSentinel MCP Tool Review
<!-- v1 -->

---

### acknowledge_sync
**Violations:** N1, N2, D1, D2
...
**Violations:** D1
**Name:** no change
**Description (revised):** Extends path coverage analysis with a cross-reference to test methods that exercise a production method. Finds covering tests by name convention (test method name contains production method name) and by direct call-site presence. Returns `BranchesToTest` (execution paths to cover), `CoveringTests` (test file, method, line), and `HasAnyCoverage` flag.
...
**Violations:** D1
**Name:** no change
**Description (revised):** Loads a `.NET` solution file into memory for persistent analysis. Must be called before any operation that requires `ErrorCode="SolutionNotLoaded"`.
...
## Summary of Rename Proposals

| Original Name | Proposed Name | Reason |
|---|---|---|
| `acknowledge_sync` | `clear_external_drift` | N1+N2: single-word "sync" undefined in context; action unclear |
| `get_external_changes` | `list_external_disk_changes` | N2: "get" + "changes" ambiguous; no indication these are on-disk drift items |
| `get_project_framework_summary` | `list_project_framework_targets` | N2: returns a listing, not a single summary artifact; "targets" is more precise |
| `list` | `list_solution_items` | N1: single-word name |
| `scan` | `run_scan_detector` | N1: single-word name |
