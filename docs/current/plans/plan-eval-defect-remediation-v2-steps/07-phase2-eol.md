# Step 2.3 — EOL detection/normalization + post-write invariant check

## Prior state

Step 2.2 already landed (verify by reading the files):
...
- Any additional targeted fixes step 2.1's repro turned up (in `ChangeAccessibilityAsync`/
...
`ReplaceNodeFormattedAsync` instead (~line 3960). This matters for step 2.4's tests, not this
...
the same and confirm zero stray CRLF. Full test gate is step 2.4's job. Proceed to
[08-phase2-fixture-tests.md](08-phase2-fixture-tests.md).
