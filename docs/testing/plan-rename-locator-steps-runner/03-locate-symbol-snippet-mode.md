# Step 2 — Add contextSnippet mode to LocateSymbol
...
`RoslynSentinel.Common/SnippetSymbolLocator.cs` already exists, with `Encode(SnippetSymbolLocator)`
and `TryDecode(string, out SnippetSymbolLocator?)` static methods (added in step 1). Confirm this
file exists and re-read its actual method signatures yourself before proceeding — do not assume
the shape from this description alone. If the file does not exist, stop and report this as a
...
- A test file under `RoslynSentinel.Tests/` covering `LocateSymbol`'s new mode

## Context

`LocateSymbol` currently finds only symbols with a declaration in the compilation's symbol table
(types and members). It cannot find a local variable or a parameter, because those never appear
in that table. This step adds a second search mode to the same tool: given a file and a snippet
of source text containing the target identifier, resolve the symbol by binding the actual syntax
at that position instead of searching by name.
...
Line numbers above are approximate — re-locate by method name if the file has moved since this
step was written.
...
1. Add a new optional parameter `string? contextSnippet = null`, following the same
   ...
   Also add `string? lineBefore = null` and `string? lineAfter = null` optional parameters, passed
   straight through, following the same style.
2. At the top of the method body, if `contextSnippet` is not null:
   ...
   - Otherwise, call the new `_symbolNavigationEngine.LocateSymbolBySnippetAsync(filePathResolved,
     contextSnippet, symbolName, lineBefore, lineAfter, cancellationToken)` instead of calling
...
- Do not implement anything in `RenameSymbol` in this step — that is a later step.
...
pre-existing tests pass. Report the build result and test pass count, and stop — do not proceed
to any other step.
