...
*Note (session 13 — contextSnippet disambiguation with lineBefore/lineAfter + MoveTypeToFile head/tail preview): `ContextHelper.FindSnippetPosition` upgraded with optional `lineBefore`/`lineAfter` disambiguation. When a snippet is ambiguous, surrounding-line context (verbatim text, trimmed Contains match) narrows the candidates. All 8 engine files (ImpactAnalyzer, GranularRefactoringEngine, MsToolAugmentEngine, DiscoveryEngine, RefactoringEngine, SymbolNavigationEngine, CodeGenerationEngine, SyntaxUpgradeEngine) and all 5 MCP wrapper files (SentinelRefactoringTools, SentinelIntelligenceTools, SentinelModernizationTools, SentinelGenerationTools, SentinelAugmentTools) updated to thread the new optional params through to tool callers. `MoveTypeToFile` preview changed from 1500-char truncation to first-10-lines + `"// ... (N lines total)"` + last-10-lines using `PreviewFileContent` helper, giving structural context (namespace header at top, closing braces at bottom). 4 new disambiguation tests added to `RegressionTests.cs`. Total: **466 passing tests, zero failures, 231 MCP tools**.*
