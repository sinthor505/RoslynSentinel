...
*Note (session 8 — AI-usability overhaul): **Removed `(int line, int column)` from all position-based MCP tools** and replaced with `string contextSnippet` — a verbatim substring the AI can paste from context without coordinate calculation. A new `ContextHelper` utility resolves snippet→position with clear errors for not-found and ambiguous matches. Changed tools: `SafeDeleteSymbol`, `IntroduceField`, `IntroduceParameter`, `IntroduceVariable` (refactoring), `GetBlastRadius`, `GetSymbolInfo`, `PreviewRenameImpact` (intelligence), `UpgradeUnboundNameof` (modernization), plus propagated through `ImpactAnalyzer`, `SymbolNavigationEngine`, `SyntaxUpgradeEngine`, and `DiscoveryEngine`. Added 4 new tools on `RefactoringEngine`: `ConvertExpressionBody` (block↔expression by member name), `ExtractConstant` (literal → named `const`, replaces all occurrences), `AnalyzeControlFlow` (always/sometimes/never-returns per method, no line numbers needed), `AnalyzeDataFlow` (read/written/captured variables per method body). Added `ContextHelperTests.cs` (4 unit tests). Total: **397 passing tests, zero failures, 218 MCP tools across 55 engines**.*
