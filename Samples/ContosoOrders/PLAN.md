# Sample Plan — ContosoOrders Cleanup Pass

> This file simulates the artifact a planning agent (or a human) would hand off to an
> implementation agent. It intentionally mirrors the shape of a real "plan → execute" handoff so
> that the test agent must read a plan and drive RoslynSentinel MCP tools to carry it out, rather
> than being told which tools to call. Do not edit this file as part of executing the plan.
