...
public class WorkspaceReadNavigationTools
{
    ...
    public Task<ToolResult<object>> SearchSolutionText(
        [Description(ToolParams.Reason)] string reason,
        [ToolOption(ToolOptionTag.Pattern, required: true)] string pattern, [ExternalInputRequired(DataTag.SourceFilepath)] string? fileGlob = null, [ToolOptionAttribute(ToolOptionTag.ResultLimit)] int maxResults = 200,
        CancellationToken cancellationToken = default)
