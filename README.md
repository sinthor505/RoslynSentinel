...
## 🚀 241 MCP Tools Across 15 Engines + Infrastructure

Roslyn Sentinel exposes **241 named MCP tools** through 7 façade classes that wrap 15 specialized analysis and transformation engines. All tools are always live; individual **feature toggles** let you silence noisy analysis rules without touching code.
...
| `extract_superclass` | Hoist common members to a new abstract base class |
...
```bash
git clone https://github.com/yourorg/RoslynSentinel
cd RoslynSentinel
...
```json
{
  "mcpServers": {
    "roslyn-sentinel": {
      "type": "stdio",
      "command": "E:/path/to/publish/RoslynSentinel.Server.exe",
      "args": []
    }
  }
}
...
Once connected, call:
```
load_solution("E:/path/to/YourSolution.sln")
```

The solution is held in RAM. All subsequent tool calls operate on the live workspace with no cold-start delay.

### 4. Multiple Solutions Simultaneously

Give each instance a unique name in your MCP config:

```json
{
  "mcpServers": {
    "sentinel-api":      { "command": "...Server.exe", "args": ["--solution=Api.sln"] },
    "sentinel-frontend": { "command": "...Server.exe", "args": ["--solution=Frontend.sln"] }
  }
}
```
...
Roslyn Sentinel is backed by **1,662 tests across 34 test batteries** (zero failures), including:
- Unit tests for every engine method (Batteries 1–27, 29–33)
- Real-solution smoke tests against a live 86-project codebase (Batteries 28, 34)
- Regression tests for every fixed bug (Batteries targeting specific CS codes and tool edge cases)

```bash
dotnet test RoslynSentinel.Tests/RoslynSentinel.Tests.csproj
# → 1,662 passed, 0 failed
```
...
- `ConvertInterfaceToAbstractClass`, `UseSpanForParsing`, `AutoParallelize`
