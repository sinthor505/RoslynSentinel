...
public class SentinelCommentingTools
{
    ...
    [Description("""
        ...
        Studio, OpenAI-compatible endpoint) called by the server itself.

        Every call always runs a mechanical seed phase first (no LLM): any member with no
        [ContentHash("Comment", ...)] at all gets stamped with a sentinel hash, which is what makes
        remaining work a queryable fact independent of any prior conversation state.
    ...
    public async Task<ToolResult<CommentingResult>> BulkComment(
        ...
        [Description("Required for scope=file, ignored otherwise.")]
