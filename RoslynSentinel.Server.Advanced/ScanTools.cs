        ...
        try
        {
            ...
            switch (detector)
            {
                ...
                case DetectorId.async_void_without_try_catch:
                    var result3 = await _asyncSafetyEngine.FindAsyncVoidWithoutTryCatchAsync(resolvedFilePath);
                ...
                case DetectorId.cancellation_token_not_forwarded:
                    var result5 = await _asyncSafetyEngine.FindCancellationTokenNotForwardedAsync(resolvedFilePath);
                ...
                case DetectorId.inconsistent_async_suffix:
                    var result7 = await _antiPatternEngine.FindInconsistentAsyncSuffixAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result7 };
                case DetectorId.mismatched_await:
                    var result8 = await _analysisEngine.DetectMismatchedAwaitAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result8 };
                case DetectorId.missing_cancellation_tokens:
                    var result9 = await _antiPatternEngine.FindMissingCancellationTokensAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result9 };
                case DetectorId.sequential_independent_awaits:
                    var result10 = await _asyncSafetyEngine.FindSequentialIndependentAwaitsAsync(resolvedFilePath);
                ...
                case DetectorId.unawaited_fire_and_forget:
                    var result17 = await _asyncSafetyEngine.FindUnawaitedFireAndForgetAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result17 };
                case DetectorId.unawaited_dispose:
                    var result18 = await _asyncSafetyEngine.FindUnawaitedDisposeAsyncAsync(resolvedFilePath);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result18 };
                case DetectorId.unobserved_task_in_field:
                    var result19 = await _asyncSafetyEngine.FindUnobservedTaskInFieldAsync(resolvedFilePath);
                ...
                case DetectorId.cas_loop_without_backoff:
                    var result21 = await _threadSafetyEngine.FindCasLoopWithoutBackoffAsync(projectName, resolvedFilePath);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result21 };
                case DetectorId.check_then_act_on_dictionary:
                    var result22 = await _threadSafetyEngine.FindCheckThenActOnDictionaryAsync(projectName, resolvedFilePath);
                ...
                case DetectorId.double_checked_locking:
                    var result24 = await _threadSafetyEngine.FindDoubleCheckedLockingAsync(projectName, resolvedFilePath);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result24 };
                case DetectorId.possible_deadlocks:
                    var result25 = await _analysisEngine.FindPossibleDeadlocksAsync(projectName, resolvedFilePath);
                ...
                case DetectorId.unsafe_lazy_init_thread:
                    var result28 = await _threadSafetyEngine.FindUnsafeLazyInitAsync(projectName, resolvedFilePath);
                ...
                case DetectorId.mutable_public_properties:
                    var result33 = await _antiPatternEngine.FindMutablePublicPropertiesAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result33 };
                case DetectorId.naming_violations:
                    var result34 = await _antiPatternEngine.FindNamingViolationsAsync(resolvedFilePath, projectName);
                ...
                case DetectorId.string_magic_values:
                    var result36 = await _antiPatternEngine.FindStringMagicValuesAsync(resolvedFilePath, projectName, 3);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result36 };
                case DetectorId.todo_fixme_comments:
                    var result37 = await _discoveryEngine.FindTodoFixmeCommentsAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result37 };

                // ── correctness ────────────────────────────────────────────────────

                case DetectorId.all_throw_sites:
                    var result38 = await _discoveryEngine.FindAllThrowSitesAsync(null, resolvedFilePath, projectName, false);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result38 };
                case DetectorId.empty_catch_blocks:
                    var result39 = await _analysisEngine.CheckForEmptyCatchBlocksAsync(resolvedFilePath, projectName);
                ...
                case DetectorId.missing_generic_constraints:
                    var result43 = await _analysisEngine.FindMissingGenericConstraintsAsync(projectName, resolvedFilePath);
                ...
                case DetectorId.multiple_out_parameter_methods:
                    var result44 = await _antiPatternEngine.FindMultipleOutParameterMethodsAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result44 };
                case DetectorId.non_exhaustive_enum_switches:
                    var result45 = await _controlFlowEngine.FindNonExhaustiveEnumSwitchesAsync(resolvedFilePath, projectName);
                ...
                case DetectorId.redundant_cast:
                    var result47 = await _analysisEngine.CheckForRedundantCastAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result47 };
                case DetectorId.resource_disposal:
                    var result48 = await _analysisEngine.OptimizeResourceDisposalAsync(resolvedFilePath, projectName);
                ...
                case DetectorId.value_type_mutation_intent:
                    var result53 = await _antiPatternEngine.FindValueTypeMutationIntentAsync(resolvedFilePath, projectName);
                ...
                case DetectorId.anti_patterns:
                    var result62 = await _antiPatternEngine.DetectAntiPatternsAsync(resolvedFilePath, projectName, null);
                ...
                case DetectorId.boxing_allocations:
                    var result64 = await _analysisEngine.FindBoxingAllocationsAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result64 };
                case DetectorId.implicit_nullable_boxing:
                    var result65 = await _performanceEngine.FindImplicitNullableBoxingAsync(resolvedFilePath);
                ...
                case DetectorId.inefficient_string_comparisons:
                    var result66 = await _analysisEngine.DetectInefficientStringComparisonsAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result66 };
                case DetectorId.linq_n1_patterns:
                    var result67 = await _performanceEngine.FindLinqN1PatternsAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result67 };
                case DetectorId.linq_redundant_where:
                    var result68 = await _performanceEngine.FindLinqRedundantWhereAsync(resolvedFilePath);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result68 };
                case DetectorId.multiple_enumeration:
                    var result69 = await _performanceEngine.FindMultipleEnumerationAsync(resolvedFilePath);
                ...
                case DetectorId.string_format_in_loops:
                    var result73 = await _performanceEngine.FindStringFormatInLoopsAsync(resolvedFilePath);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result73 };
                case DetectorId.use_frozen_collections:
                    var result74 = await _codeStyleEngine.FindUseFrozenCollectionsAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result74 };

                // ── security ───────────────────────────────────────────────────────

                case DetectorId.hardcoded_paths:
                    var result75 = await _securityEngine.FindHardcodedPathsAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result75 };
                case DetectorId.reflection_usage:
                    var result76 = await _analysisEngine.DetectReflectionUsageAsync(resolvedFilePath, projectName);
                ...
                case DetectorId.sql_injection:
                    var result78 = await _securityEngine.CheckForSqlInjectionAsync(resolvedFilePath, projectName);
                ...
                case DetectorId.layer_violations:
                    var result89 = await _architecturalEngine.DetectLayerViolationsAsync(projectName, resolvedFilePath);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result89 };
                case DetectorId.long_parameter_list:
                    var result90 = await _antiPatternEngine.FindLongParameterListAsync(resolvedFilePath, projectName, 4);
                ...
                case DetectorId.primitive_obsession:
                    var result92 = await _antiPatternEngine.FindPrimitiveObsessionAsync(resolvedFilePath, projectName);
                    return new SentinelCallToolResult<object>() { IsSuccess = true, SuccessData = result92 };
                case DetectorId.structural_smells:
                    var result93 = await _projectStructureEngine.FindStructuralSmellsAsync(ProjectStructureEngine.StructuralSmellType.All, projectName, resolvedFilePath);
