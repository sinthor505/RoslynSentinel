...
public class SemanticRefactoringLibrary
{
    ...
    public async Task<DocumentEditResult> ConvertPropertyToMethodsAsync(FilePathWrapper filePath, string className, string propertyName, CancellationToken cancellationToken = default)
    {
        ...
        return new DocumentEditResult { Outcome = EditOutcome.Modified, UpdatedText = await RoslynFormattingHelper.ReplaceNodeFormattedAsync(document, root!, classNode, newClass, cancellationToken), FilePath = filePath };
    }
    ...
    public async Task<DocumentEditResult> WrapInUsingAsync(FilePathWrapper filePath, int startLine, int endLine, string disposalName, CancellationToken cancellationToken = default)
    {
        ...
        return new DocumentEditResult { Outcome = EditOutcome.Modified, UpdatedText = await RoslynFormattingHelper.ReplaceNodeFormattedAsync(document, root!, parentBlock, newBlock, cancellationToken), FilePath = filePath };
    }
