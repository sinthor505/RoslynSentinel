...
public class McpTasksHarnessTests
{
    ...
    public async Task TaskCapableClient_CallingTaskEligibleTool_ReturnsCreateTaskResult()
    {
        ...
        {
            Name = "Features",
            Arguments = ToArguments(new Dictionary<string, object?> { ["reason"] = "test message", ["action"] = "list", ["delaySeconds"] = 5 }),
        };
    ...
    public async Task TaskCapableClient_PollingToCompletion_MatchesSynchronousResult()
    {
        ...
        var polledResult = await _client.CallToolWithPollingAsync(
            ...
            {
                Name = "Features",
                Arguments = ToArguments(new Dictionary<string, object?> { ["reason"] = "test message", ["action"] = "list", ["delaySeconds"] = 3 }),
            },
            cancellationToken: TestContext.CurrentContext.CancellationToken);
    ...
    public async Task CancelTask_MidFlight_StopsBeforeCompletionAndReportsCancelled()
    {
        ...
        {
            Name = "Features",
            Arguments = ToArguments(new Dictionary<string, object?> { ["reason"] = "test message", ["action"] = "list", ["delaySeconds"] = 15 }),
        };
